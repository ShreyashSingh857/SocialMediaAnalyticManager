...
export const AnalyticsLayout: React.FC = () => {
    ...
    const { loading, error } = useYouTubeData();
...
const TabLink = ({ to, label, isActive }: { to: string; label: string; isActive: boolean }) => (
    ...
    >
        {label}
        {isActive && (
            <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1/2 h-0.5 bg-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.8)] rounded-full"></span>
        )}
    </NavLink>
);
